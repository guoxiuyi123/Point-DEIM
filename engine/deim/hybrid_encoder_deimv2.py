...
class HybridEncoderV2(nn.Module):   
    ...
    def __init__(self,   
        ...
        if downsample_op == 'freqlawds':
            from ..extre_module.custom_nn.downsample.FreqLAWDS import FreqLAWDS
            SCDown_Conv = FreqLAWDS(hidden_dim, hidden_dim)
        else:
    ...
    def forward(self, feats):  
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            fused_feat = (downsample_feat + feat_height) if self.fuse_op == 'sum' else torch.concat([downsample_feat, feat_height], dim=1)
