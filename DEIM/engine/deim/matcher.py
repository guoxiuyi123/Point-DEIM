...
@register()
class HungarianMatcherPointSup(nn.Module):
    __share__ = ['use_focal_loss', ]
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0):
        ...
        self.cost_point = weight_dict.get('cost_point', 0.0)

        self.use_focal_loss = use_focal_loss
        self.alpha = alpha
        self.gamma = gamma

        assert self.cost_class != 0 or self.cost_bbox != 0 or self.cost_giou != 0 or self.cost_point != 0, "all costs cant be 0"
        self.epoch = -1
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False, epoch=0, num_queries_list=None):
        ...
        if self.cost_class != 0:
            if self.use_focal_loss:
                p = out_prob[:, tgt_ids]
                neg_cost_class = (1 - self.alpha) * (p ** self.gamma) * (-(1 - p + 1e-8).log())
                pos_cost_class = self.alpha * ((1 - p) ** self.gamma) * (-(p + 1e-8).log())
                cost_class = pos_cost_class - neg_cost_class
            else:
                cost_class = -out_prob[:, tgt_ids]
        ...
        cost_point = 0.0
        if self.cost_point != 0:
            cost_point = torch.cdist(out_bbox[:, :2], tgt_points, p=1)

        C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + self.cost_giou * cost_giou + self.cost_point * cost_point
        C = C.view(bs, num_queries, -1).cpu()
        ...
        if return_topk:
            return {'indices_o2m': HungarianMatcher.get_top_k_matches(self, C, sizes=sizes, k=return_topk, initial_indices=indices_pre)}
