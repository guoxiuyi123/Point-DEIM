        ...
        for k, v in self.__dict__.items():    
            if hasattr(v, 'load_state_dict') and k in state:
                ...
                try:
                    ...
                    if isinstance(v, nn.Module) and isinstance(state[k], dict):
                        incompatible = v.load_state_dict(state[k], strict=False)
                        logger.warning(f'Load {k}.state_dict with strict=False, missing={len(incompatible.missing_keys)}, unexpected={len(incompatible.unexpected_keys)}')
                    else:
                        raise e
